    ...
    public class Account
    {
        ...
        public string GetLoginVCode()
        {
            ...
            do
            {
                    ...
                    Stream stream =
                        HttpRequest.Create("https://dynamic.12306.cn/otsweb/passCodeAction.do?rand=sjrand",
        ...
        /// </summary>
        /// <returns></returns>
        public List<TrainItemInfo> QueryTrainInfos()
        {
            ...
            if(IsLogin)
            {
                ...
                string content = HttpRequest.Create(url, "https://dynamic.12306.cn/otsweb/order/querySingleAction.do?method=init", Cookie, HttpMethod.GET, "").GetString();
                ...
                return ToTrainItemInfos(content);
            }
            return list;
        }
        ...
        /// 下订单后等待订单结束
        /// </summary>
        /// <exception cref="System.Exception">如下单失败异常</exception>
        /// <returns>返回订单编号</returns>
        public WaitResponse GetOderId()
        {
            WaitResponse waitResponse=null;
            do
            {
                string resContent;
                waitResponse = GetWaitResponse(out resContent);
                if (waitResponse != null)
                {
                    if (!string.IsNullOrEmpty(waitResponse.OrderId)) return waitResponse;
                    if (!string.IsNullOrEmpty(waitResponse.ErrorCode) && !string.IsNullOrEmpty(waitResponse.Msg))
                    {
                        throw new Exception(waitResponse.Msg);
                    }
                }
        ...
        public WaitResponse GetWaitResponse(out string rspContent)
        {
            ...
            WaitResponse waitResponse = waitResponseContent.ToJsonObject<WaitResponse>();
            if(waitResponse==null)
            {
                Common.LogUtil.Log(waitResponseContent);
        ...
        /// </summary>
        /// <returns></returns>
        private static string GetVCodeByForm(Image image)
        {
           VCodeForm vCodeForm=new VCodeForm(image);
            if(vCodeForm.ShowDialog()==DialogResult.OK && !string.IsNullOrEmpty(vCodeForm.Value))
            {
                return vCodeForm.Value.Trim();
            }
            return "";
        }

        public static List<TrainItemInfo> ToTrainItemInfos(string content)
        {
            ...
            for (int i = 0; i < records.Length; i ++)
            {
                ...
                TrainItemInfo itemInfo = new TrainItemInfo()
                                             {
                                                 No=contents[0],
                                                 TrainNo = Common.HtmlUtil.RemoveHtml(contents[ 1]),
                                                 StartStation =Common.HtmlUtil.HtmlDecode(Common.HtmlUtil.RemoveHtml(contents[2])).Trim(),
                                                 EndStation =Common.HtmlUtil.HtmlDecode( Common.HtmlUtil.RemoveHtml(contents[3])).Trim(),
                                                 lishi = Common.HtmlUtil.RemoveHtml(contents[4]),
                                                 ShangwuSeat = Common.HtmlUtil.RemoveHtml(contents[5]),
                                                 TedengSeat = Common.HtmlUtil.RemoveHtml(contents[6]),
                                                 YidengSeat = Common.HtmlUtil.RemoveHtml(contents[7]),
                                                 ErdengSeat = Common.HtmlUtil.RemoveHtml(contents[8]),
                                                 GaojiRuanwoSeat = Common.HtmlUtil.RemoveHtml(contents[9]),
                                                 RuanwoSeat = Common.HtmlUtil.RemoveHtml(contents[10]),
                                                 YingwoSeat = Common.HtmlUtil.RemoveHtml(contents[11]),
                                                 RuanzuoSeat = Common.HtmlUtil.RemoveHtml(contents[12]),
                                                 YingzuoSeat = Common.HtmlUtil.RemoveHtml(contents[13]),
                                                 WuzuoSeat = Common.HtmlUtil.RemoveHtml(contents[14]),
                                                 OtherSeat = Common.HtmlUtil.RemoveHtml(contents[15]),


                                                 Tag = Common.HtmlUtil.RemoveHtml(contents[16])
                                             };
